...
async def start_MainKeyboard(message: Message):
    ...
    await bot.send_message(
        message.from_user.id, text=text, reply_markup=kb.MainKeyboard
    )
...
async def MainKeyboard_func(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        "<b>Главное меню:</b>", reply_markup=kb.MainKeyboard
    )
...
async def keyExchange_func(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(
        "<b>Курс валют:</b>", reply_markup=kb.keyExchange
    )
...
async def keyUSD_func(callback_query: types.CallbackQuery):
    xml_response = etree.fromstring(
        requests.get("http://www.cbr.ru/scripts/XML_daily.asp").text.encode("1251")
    )
    curs = xml_response.find("Valute[@ID='R01235']/Value").text
    await callback_query.message.edit_text(
        text=f"1 доллар {curs} рублей", reply_markup=kb.keyUSD
    )
...
async def keyUSD_func(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(
        text=f"1 eвро {curs} рублей", reply_markup=kb.keyEUR
    )
...
async def keyFilms_func(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        text="<b>Фильмы:</b>", reply_markup=kb.keyFilms
    )
...
async def Swatchthis_func(callback_query: types.CallbackQuery):
    ...
    if data is None:  # если нет выводим сообщение с кнопкой
        await callback_query.message.edit_text(
            text="<b>Фильмы отсутствуют:</b>", reply_markup=kb.filmfavorites
        )
    else:
...
async def addfavorites_func(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        text="Для добавления воспользвуйтесь командой /addfavoritefilm"
    )
...
async def keyTasks_func(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        text="<b>Задачи:</b>", reply_markup=kb.keyTasks
    )
...
async def keyList_func(callback_query: types.CallbackQuery):
    ...
    if data is None:  # если нет выводим сообщение с кнопкой
        await callback_query.message.edit_text(
            text="<b>Задачи отсутствуют 🗒</b>", reply_markup=kb.keyList
        )
...
async def keyAddFunc(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        text="Для добавления воспользвуйтесь командой /addtask"
    )
...
async def keydeletealltasksFunc(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(
        text="Все задачи удалены ✅", reply_markup=kb.keydeletealltasks
    )
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await asyncio.sleep(1)
        await bot.send_message(
            message.chat.id, text=film_desc, reply_markup=kb.FindName
        )
        # Finish conversation
        data.state = None
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        # And send message
        await bot.send_message(
            message.chat.id, "Фильм добавлен ✅", reply_markup=kb.filmfavorites
        )
        user_id = message.chat.id
        name = data["film_name"]
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        # And send message
        await bot.send_message(
            message.chat.id, "Задача добавлена ✅", reply_markup=kb.keydeletealltasks
        )
        user_id = message.chat.id
