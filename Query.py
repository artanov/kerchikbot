...
class Scripts:
    ...
    def select_favoritefilms(people_id):
        cursor.execute(
            f"""SELECT favorites_name FROM filmsfavorites where user_id = {people_id}"""
        )
        query_results = cursor.fetchall()
        text = "\n\n".join([", ".join(map(str, x)) for x in query_results])
        return str(text)
    ...
    def add_favoritefilm(name_film, user_id):
        ...
        with connect.cursor() as cursor:
            cursor.execute(
                """INSERT INTO filmsfavorites ("favorites_name",user_id) VALUES(%s,%s);""",
                (name_film, user_id),
            )
        return connect.commit()
    ...
    def delete_all_favorites(people_id):
        ...
        with connect.cursor() as cursor:
            cursor.execute(
                f"""Delete from filmsfavorites where user_id = {people_id}"""
            )
        return connect.commit()
